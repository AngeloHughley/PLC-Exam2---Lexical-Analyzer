...
public class Lexical {
	...
	public static void main(String[] args) {
		...
		try {
			...
			while(scan.hasNextLine()) {
				...
				for(int i = 0; i < eachWord.length; i++) {
					...
					if(a.errorChecker) {
						...
						if(Pattern.matches("[0-9]", eachWord[i])) {
			...
			a.curlyBraceCheck(a.curlyBrace, numberLine); //Check for a even number of brackets. If not even, generate a error in the last line
		}
		catch(Exception e) {
